...
def connect():
    FTP_IP = '187.45.242.11' ####### INSERT FTP HOST IP ADDRESS
    ...
    PORT = 21 ####### CHANGE PORT IF NEEDED (DEFAULT IS 21)
...
def sendFiles():
    ...
    try:
        ftp.mkd(f"/path/{folder1}") ########### EDIT PATH HERE
        ftp.cwd(f'/path/{folder1}') ########### EDIT PATH HERE
...
schedule.every().day.at('17:32').do(execSchedule) ##### CHANGE HERE TO TIME THAT YOU WANT TO SEND
